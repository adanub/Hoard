...
So I looked around to see if there were any tools out there that already existed for doing this,
...
Hoard lets you pick any board on Pinterest you can access and saves it into a "project": a local
folder on your machine. More details below 👇

<!-- Generated by tools/preview/app-preview.py, which draws the same mock as the website's hero from
     the same seeded palette — so the two never drift apart. Re-run it after changing either. It is a
     drawing, not a screenshot: a screenshot would have to contain somebody's real boards (see the
     "no real personal data" rule), and this stays crisp at any width for ~15 KB. The <picture> gives
     GitHub a light and a dark copy so the preview follows the reader's theme. -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="assets/preview/app-dark.svg">
  <img src="assets/preview/app-light.svg" width="820"
...
Hoard isn't a replacement for Pinterest. It keeps a backup of what you've already saved to your boards
so you can browse it offline; finding new things to save still happens on Pinterest.
...
| **Syncs cheaply** | A sync walks a board newest-first and stops as soon as it hits images you already have, so picking up 3 new pins costs a page or two rather than a re-crawl of the whole board. Or at least it tries to, anyways - if you've moved stuff around on Pinterest after the sync for example, then it might miss some things on the next sync. There's a full sync option for cases like that. |
| **Merges boards** | Point several Pinterest boards at one local board; they gather into the same grid. |
| **Backs itself up** | Point a project at a second folder: an external drive, a NAS share, a network-synced folder. After the first backup sync, only the changes move. A backup for your backup 🤯 |
| **Exports to plain folder** | Hoard stores a project in a layout that's fast for it to read and write, so browsing stays quick. That layout isn't much fun for a human to navigate though, so there's an export button that spits out a copy arranged the way you'd actually want to click through it in your file explorer. You don't need Hoard to open it. |
...
The links below always download the latest release. For other versions, visit the
[GitHub releases page](https://github.com/adanub/Hoard/releases).

| Platform | Download | |
| --- | --- | --- |
| Windows (x64) | [`Hoard-installer-win-x64.exe`](https://github.com/adanub/Hoard/releases/latest/download/Hoard-installer-win-x64.exe) | **Recommended.** Installs for your user only, with no admin prompt, and updates itself |
| Windows (x64) | [`Hoard-portable-win-x64.zip`](https://github.com/adanub/Hoard/releases/latest/download/Hoard-portable-win-x64.zip) | Portable: unzip and run. No installer, and it can't update itself |
...
> first time you run it. Click **More info → Run anyway**. That's once per download; updates installed from
...
>
> **Intel Macs aren't built.** I only intend to support Apple Silicon (M1 or newer), though you're welcome
> to try building it yourself from the source.

### Updating

The Windows installer build and the macOS app check for a new release at startup and **ask** before
installing anything; declining leaves it alone. You can change both behaviours in
**⚙ Settings → Updates**:

- **Check for updates automatically** (on by default). Turn it off and Hoard never phones home.
- **Install updates automatically** (off by default). Hoard downloads in the background and applies the
  update the next time you close it, without asking.
- **Check for updates** / **Install update**. Do either by hand, whenever you like.
...
one build that can't update itself; for that one, download the new release over the old one. Settings
...
missing from disk. A quick sync structurally can't see any of those.
...
Hoard moves exactly the new blobs instead of comparing thousands of files, which over SMB is the
difference between a couple of seconds and ten minutes. There's still a full sync option, just in case.

**Repair backup** is the thorough pass. Use it if the backup folder has been changed from outside: files
deleted, a drive half-restored, that sort of thing. A normal sync can only see damage that something logged.

Two folders carrying the same project marker are copies of the *same* archive, so syncing them against
each other is the intended workflow. It's how you keep a local working copy and a NAS copy in step, and
it's also how a machine heals
...
your OS into `tools/gallery-dl/` the first time. It only fetches when the file is *absent*, so to refresh
a copy that Pinterest has broken, run `pwsh tools/fetch-gallery-dl.ps1`. To build offline, pass
...
If imports start failing across the board, the usual cause is Pinterest having changed
something. Refresh gallery-dl with `pwsh tools/fetch-gallery-dl.ps1` before assuming it's Hoard.
...
- **Tags are usually empty**, because Pinterest's sidecars rarely carry any in practice.
...
and it only ever fetches content the account you're signed in as can already see and download manually.
